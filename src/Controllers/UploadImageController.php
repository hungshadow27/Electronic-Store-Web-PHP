            ...
            if (isset($_FILES['fileUpload'])) {
                ...
                if ($allowUpload) {
                    ...
                    if (move_uploaded_file($_FILES["fileUpload"]["tmp_name"], $target_file)) {
                        ...
                        $usermodel->updateUserImage($user->getId(), basename($_FILES["fileUpload"]["name"]));
                        $user->setImage(basename($_FILES["fileUpload"]["name"]));
                        $_SESSION['USER'] = serialize($user);
                        echo "<script>alert('Bạn đã cập nhật ảnh đại diện thành công!');
